...
// FIXME: this is broken; it does not namespace entries by ledger node ID
  ...
  async get({blockHeight} = {}) {
    try {
      ...
      if(blockHeight === undefined) {
        ...
        const fn = () => this.storage.events.getEffectiveConfig({blockHeight});
        result = await lruCache.memoize({key: blockHeight, fn});
      }
