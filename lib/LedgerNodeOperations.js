...
module.exports = class LedgerNodeOperations {
  ...
  async add({operation}) {
    if(!operation) {
      throw new BedrockError('`operation` is required.', 'TypeError');
    }
  ...
  async validate(operation) {
    if(!(operation && typeof operation === 'object')) {
      throw new BedrockError('Operation must be an object.', 'TypeError');
    }
...
function _validateContext(context) {
  ...
  if(typeof context === 'string' && context !== wlContextUrl) {
    throw new BedrockError(`Operation context must be "${wlContextUrl}"`,
      'SyntaxError', {context});
  }
  if(Array.isArray(context) && context[0] !== wlContextUrl) {
    throw new BedrockError(`Operation context must contain "${wlContextUrl}"` +
      ' as the first element.', 'SyntaxError', {context});
  }
}
