...
module.exports = class LedgerNodePeers {
  ...
  async getAll({
    minReputation = 0, maxReputation, backoffUntil, localBlockHeight,
    ...
    if(localBlockHeight !== undefined) {
      query['peer.status.requiredBlockHeight'] = {$lte: localBlockHeight};
    }
  ...
  async getLRU({
    ...
    backoffUntil = Date.now(), localBlockHeight = 0,
    ...
    const query = {
      ...
      'peer.status.requiredBlockHeight': {$lte: localBlockHeight},
