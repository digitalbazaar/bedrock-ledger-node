...
const {COLLLECTION_NAME} = require('./ledgerNodePeerCollection');
...
module.exports = class LedgerNodePeers {
  ...
  async add({peer}) {
    const collection = database.collections[COLLLECTION_NAME];
    const now = Date.now();
    const peerRecord = {
      meta: {
        created: now,
  ...
  async get({id}) {
    ...
    const result = await collection.findOne(query, {_id: 0, peer: 1});
