...
module.exports = class LedgerNodeRecords {
  ...
  async get({maxBlockHeight, recordId}) {
    // if maxBlockHeight is given, we check a cache to see if we at least have
    // that block height based on the last time we called
    // `_getLatestBlockHeight`. If not, we call it again and cache its
    // result; if we don't have that block height, we readjust the block
    // height down to the latest block height.
    let latestBlockHeight = await this._getLatestBlockHeight();
    ...
    if(maxBlockHeight === undefined) {
      ...
      // only if `maxBlockHeight` is still greater than `latestBlockHeight`
      this._latestBlockSummaryCache.delete('latestBlockSummary');
      latestBlockHeight = await this._getLatestBlockHeight();
  ...
  async _getLatestBlockHeight() {
    ...
    // be called often
    const fn = () => this.ledgerNode.storage.blocks
      .getLatestSummary(this.ledgerNode);
    const latestBlockSummary = await this._latestBlockSummaryCache.memoize({
      key: 'latestBlockSummary', fn});

    return latestBlockSummary.eventBlock.block.blockHeight;
  }
