...
module.exports = class LedgerNodeWorkSession {
  constructor({schedulerId, ledgerNode, onFinish}) {
    ...
    this.start = callbackify(this.start.bind(this));
  }
  ...
  async start(maxAge, fn) {
    ...
    try {
      ...
      try {
        // FIXME: remove promisify once consensus methods are updated
        await promisify(fn)(this);
      } catch(e) {
        logger.error(
  ...
  async _reserve() {
    ...
    const update = {
      $set: {
        'meta.workSession': {id: this.id, expires: expires},
