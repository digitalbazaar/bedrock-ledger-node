 ...
 *
 * TODO: change API to `create` instead of `add`?
 * TODO: add a `clone` or `join` method?
 * TODO: could either make the clone/join method require the genesis block
 *   as a parameter (and optionally take a set of web ledger agent URLs to
 *   store for the consensus plugin to use), or it could actually use a
 *   web ledger agent client to obtain the genesis block... but need to be
 *   careful here with circular dependencies)
 *
 ...
 // ensure consensus method is specified
  ...
  async.auto({
    ...
    writeConfig: ['createNode', (results, callback) =>
      // FIXME: send `results.createNode` instead of `results.initStorage`
      // and remove from `options`
      results.getConsensus.api.events.add(
        configEvent, results.initStorage, {
          genesis: true,
          ledgerNode: results.createNode
        }, callback)
    ]
  }, (err, results) =>
