...
bedrock.events.on('bedrock-mongodb.ready', callback => async.auto({
  ...
  createIndexes: ['openCollections', (results, callback) =>
    database.createIndexes([{
      ...
      collection: 'ledgerNode',
      fields: {'meta.workSession': 1, 'meta.updated': 1},
      options: {unique: false, background: false}
    }, {
      collection: 'ledgerNode',
      fields: {'meta.workSession.expires': 1, 'meta.updated': 1},
      options: {unique: false, background: false}
    }], callback)
  ],
...
api.add = function(actor, options, callback) {
  ...
  if(!(configEvent && typeof configEvent === 'object' &&
    ...
    jsonld.hasValue(
      configEvent.ledgerConfiguration, 'type', 'WebLedgerConfiguration'))) {
    return callback(new BedrockError(
      'The provided configuration event is malformed. ' +
      'An object with `type` set to `WebLedgerConfiguration` does not exist.',
      'DataError',
      {httpStatusCode: 400, public: true, configEvent}
    ));
  }
  const wlConfig = configEvent.ledgerConfiguration;
  ...
  const node = {
    id: 'urn:uuid:' + uuid(),
    ledger: configEvent.ledgerConfiguration.ledger,
  ...
  start(maxAge, fn, callback) {
    ...
    async.auto({
      ...
      release: ['execute', (results, callback) => {
        ...
        const update = {
          $set: {
            // TODO: `null` or back to `self.schedulerId` and the
            // old expires date?
...
api.consensus._scheduleConsensusWork = (job, callback) => {
  ...
  function claimLedgerNode(callback) {
    // TODO: should really be marking the *most* out of date ledger node that
    //   can be found, not just picking any that haven't been updated since
    //   the scheduler started
    const query = {
      'meta.deleted': {$exists: false},
      'meta.updated': {$lte: start},
      $or: [
        {'meta.workSession': null},
