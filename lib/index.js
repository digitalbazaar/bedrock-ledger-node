...
api.add = (actor, configBlock, options, callback) => {
  ...
  async.auto({
    ...
    // FIXME: skipping check existing. Does this check need to happen?
    checkExisting: ['checkPermission', (results, callback) => callback()],
...
api.get = (actor, ledgerNodeId, options, callback) => {
  ...
  async.auto({
    ...
    createNode: ['initStorage', (results, callback) => {
