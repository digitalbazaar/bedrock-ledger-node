...
api.use = (capabilityName, capabilityValue) => {
  ...
  if(callback) {
    if(!plugin) {
      return callback(new BedrockError('Plugin not found.', 'NotFoundError'));
    }
  ...
  async.auto({
    ...
    getStorage: ['checkPermission', (results, callback) =>
      api.use(options.storage, (err, result) => {
        if(err) {
          return callback(new BedrockError(
            'Invalid storage system specified.', 'DataError', {
              httpStatusCode: 400, public: true, storage: options.storage
            }));
        }
