...
api.use = (capabilityName, capabilityValue) => {
  if(!capabilityName) {
    return new TypeError('`capabilityName` is a required.');
  }
  ...
  if(arguments.length === 1 || callback) {
    ...
    if(callback) {
      callback(null, plugin.api);
    }
    return plugin.api;
  }
  if(!capabilityValue) {
    return new TypeError('`capabilityValue` is a required.');
  }
  if(plugin) {
    return new Error('Plugin already registered: ' + capabilityName);
  }
