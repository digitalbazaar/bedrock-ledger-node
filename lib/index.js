...
api.add = (actor, options, callback) => {
  ...
  if(options.ledgerConfiguration) {
    ...
    const configEvent = _.find(options.genesisBlock.event, obj => {
      return obj.hasOwnProperty('ledgerConfiguration');
    });
  ...
  async.auto({
    ...
    getConsensus: ['insert', (results, callback) =>
      _use(ledgerConfiguration.consensusMethod, callback)],
...
api.get = (actor, ledgerNodeId, options, callback) => {
  ...
  async.auto({
    ...
    getStorage: ['checkPermission', (results, callback) =>
      _use(results.find.ledgerNode.storage.plugin, callback)],
    ...
    initConsensus: ['getConfig', (results, callback) => {
      ...
      const {ledgerConfiguration} = results.getConfig.event;
      _use(ledgerConfiguration.consensusMethod, callback);
    }],
  ...
  async.auto({
    ...
    hash: ['canonize', (results, callback) => {
      const hash = crypto.createHash(
        'sha256').update(results.canonize, 'utf8').digest();
