 ...
 // ensure consensus method is specified
  ...
  async.auto({
    ...
    writeConfig: ['getConsensus', 'createNode', (results, callback) =>
      results.getConsensus.events.add(
  ...
  async.auto({
    ...
    createNode: ['initStorage', 'initConsensus', (results, callback) => {
      callback(null, new LedgerNode({
        ...
        storage: results.initStorage,
        consensus: results.initConsensus
      }));
    }]
 ...
 */
api.consensus.process = (method, session, fn, callback) => {
  async.auto({
    ...
    reconcile: ['reserve', (results, callback) => {
      ...
      // TODO: is a ledgerNode given instead?
      fn(results.reserve.ledgerId, session, callback);
    }],
...
api.consensus._reserve = (method, session, callback) => {
  const plugins = Object.keys(plugins).filter(x => plugins[x] === 'storage');

  // TODO: loop
  let reserved = null;
  async.until(() => !reserved, loop => {
    // pick a storage plugin at random
    const idx = plugins[Math.floor(Math.random() * plugins.length)];
    const plugin = plugins[idx];
    plugins.splice(idx, 1);

    // TODO: get storage plugin

    // TODO: try to reserve a ledger with the storage API
    ...
    loop();
  }, err => callback(err, reserved));
};

api.consensus._release = (reserved, callback) => {
  // TODO: implement me
  // reserved.storage.consensus.release(..., callback);
  callback();
};
