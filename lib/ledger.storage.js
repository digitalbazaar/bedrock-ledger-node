...
api.createLedger = function(actor, ledgerConfigEvent, options, callback) {
  ...
  async.auto({
    ...
    fetchSigningKey: ['checkKeyAuthorization', function(callback) {
      // get the creator key
      request(ledgerConfigEvent.signature.creator, function(err, res, body) {
        if(err) {
          return callback(err);
        }
        if(res.statusCode !== 200) {
          return callback(new BedrockError(
            'Could not fetch public key data for signature.',
            'Forbidden', {
              httpStatusCode: 403,
              signingKey: ledgerConfigEvent.signature.creator,
              public: true
            }
          ));
        }
    ...
    checkSignature: ['fetchSigningKey', function(callback, results) {
      // FIXME: Don't skip signature check
      return callback();
      // FIXME: This is not safe, the bi-directional ownership of key->owner
      // and owner-> key should be checked
      var signingKey = results.fetchSigningKey.id;
      var signingKeyOwner = results.fetchSigningKey.owner;
      // check to see that the signature is valid
      jsigs.verify(ledgerConfigEvent, {
        publicKey: signingKey,
        publicKeyOwner: signingKeyOwner,
