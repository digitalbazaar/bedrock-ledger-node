...
const nodeRecordLruCache = new LruCache({
  max: 100,
  disposeOnGet: true
});
...
/**
 * Gets a ledger node record given a ledgerNodeId.
 *
 * @param actor - the actor performing the action.
 * @param ledgerNodeId - the URI of the ledger node.
 *
 * @return a Promise that resolves to a LedgerNode Record.
 */
/* eslint-disable-next-line no-unused-vars */
api.getNodeRecord = async (actor, ledgerNodeId) => {
  const fn = () => _findLedgerNode({actor, ledgerNodeId});

  let result;
  // only use cache if no actor because actor permissions may have changed
  if(!actor) {
    result = await nodeRecordLruCache.memoize({key: ledgerNodeId, fn});
  } else {
    result = await fn();
  }
