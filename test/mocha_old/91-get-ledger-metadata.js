...
describe.skip('OLD DISABLED getLedgerMetaData API', () => {
  before(done => {
  ...
  describe('regularUser as actor', () => {
    ...
    it('creates a ledger and gets metadata', done => {
      ...
      async.auto({
        ...
        getMeta: ['create', (results, callback) => brLedger.getLedgerMetadata(
          actor, mockLedger.config.ledgerConfig.name, {}, (err, result) => {
            ...
            const eventNumber = helpers.getEventNumber(le.id);
            ...
            should.exist(ne.id);
            ne.id.should.equal(lc.id + '/events/' + (eventNumber + 1));
            callback();
          })
        ]
      }, done);
